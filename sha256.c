...
void calculate_sha256(const unsigned char *data, unsigned char *hash, unsigned int *hash_len) {
...
// Function to read 4KB pages from a file and calculate their SHA-256 hash
void process_file_sha256(const char *file_path) {
    ...
    unsigned char hash[SHA256_DIGEST_LENGTH];  // SHA-256 produces a 32-byte hash
    unsigned int hash_len;
    size_t read_size;
    int page_count = 0;

    // Read the file 4KB at a time and calculate SHA-256 for each page
    while ((read_size = fread(buffer, 1, PAGE_SIZE, file)) == PAGE_SIZE) {
        double start_time = get_time_in_seconds();
        calculate_sha256(buffer, hash, &hash_len);  // Calculate SHA-256 hash
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        iteration_time = 0.0;
        process_file_sha256(argv[1]);  // Process the file
