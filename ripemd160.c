...
// Function to calculate RIPEMD-160 hash using EVP interface
void calculate_ripemd160(const unsigned char *data, unsigned char *hash, unsigned int *hash_len) {
    EVP_MD_CTX *ctx = EVP_MD_CTX_new();             // Create a new context
    EVP_DigestInit_ex(ctx, EVP_ripemd160(), NULL);  // Initialize for RIPEMD-160
...
// Function to read 4KB pages from a file and calculate their RIPEMD-160 hash
void process_file_ripemd160(const char *file_path) {
    ...
    unsigned char hash[20];  // RIPEMD-160 produces a 20-byte hash
    unsigned int hash_len;
    size_t read_size;
    int page_count = 0;

    // Read the file 4KB at a time and calculate RIPEMD-160 for each page
    while ((read_size = fread(buffer, 1, PAGE_SIZE, file)) == PAGE_SIZE) {
        double start_time = get_time_in_seconds();
        calculate_ripemd160(buffer, hash, &hash_len);  // Calculate RIPEMD-160 hash
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        iteration_time = 0.0;
        process_file_ripemd160(argv[1]);  // Process the file
