...
// Function to calculate SHA3-256 hash using EVP interface
void calculate_sha3_256(const unsigned char *data, unsigned char *hash, unsigned int *hash_len) {
    EVP_MD_CTX *ctx = EVP_MD_CTX_new();  // Create a new context
    EVP_DigestInit_ex(ctx, EVP_sha3_256(), NULL);  // Initialize for SHA3-256
...
// Function to read 4KB pages from a file and calculate their SHA3-256 hash
void process_file_sha3_256(const char *file_path) {
    ...
    unsigned char hash[EVP_MAX_MD_SIZE];  // SHA3-256 hash size is 32 bytes
    unsigned int hash_len;
    size_t read_size;
    int page_count = 0;

    // Read the file 4KB at a time and calculate SHA3-256 for each page
    while ((read_size = fread(buffer, 1, PAGE_SIZE, file)) == PAGE_SIZE) {
        double start_time = get_time_in_seconds();
        calculate_sha3_256(buffer, hash, &hash_len);  // Calculate SHA3-256 hash
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        iteration_time = 0.0;
        process_file_sha3_256(argv[1]);  // Process the file
